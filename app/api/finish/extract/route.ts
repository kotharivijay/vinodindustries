...
export interface ExtractedFinishSlip {
  slipNo: string | null
  date: string | null
  lotNo: string | null
  than: number | null
...
const SYSTEM_PROMPT = `You are extracting data from a textile finish/center process slip (Indian textile industry).
...
Return ONLY a valid JSON object with these exact fields:
{
  ...
  "marka": [
    { "lotNo": "lot number like PS-1325 or AJ-325", "than": numeric than/piece count or null, "meter": numeric meter value or null }
  ],
  "chemicals": [
    { "name": "chemical name", "quantity": numeric amount or null, "unit": "kg or gram" }
  ],
  "mandi": numeric mandi value in liters or null,
...
Rules:
- The "marka" field lists lot numbers with their than (piece) quantities and meter values. Extract ALL lot entries. Lot numbers follow the pattern: alphabetic prefix + hyphen + number (e.g. PS-1325, AJ-325). If there's only one lot, still return it as a single-item array.
- chemicals array must include ALL chemicals listed on the slip
- For chemical quantity: extract the raw numeric value exactly as written. If >10, unit is "gram". If <=10, unit is "kg". If clearly written on slip, use that.
- mandi is the water/solution quantity, usually in liters
- If a field is not visible or unclear, use null
- For lot numbers, preserve exact format. If no hyphen, insert one between letters and digits
- For date, convert to DD/MM/YYYY format
- Return ONLY the JSON object, no explanation`

function parseJSON(text: string): ExtractedFinishSlip {
  ...
  if (!raw.marka && raw.lotNo) {
    raw.marka = [{ lotNo: raw.lotNo, than: raw.than ?? null, meter: raw.meter ?? null }]
  }
  ...
  // Ensure mandi is numeric or null
  raw.mandi = raw.mandi != null ? parseFloat(raw.mandi) || null : null
  ...
  if (raw.chemicals) {
    raw.chemicals = raw.chemicals.map((c: any) => {
      ...
      if (qty != null && qty > 10) {
        ...
        // AI said gram but qty <= 10 -> it's actually kg
        unit = 'kg'
      }
...
async function extractWithOpenAI(imageBase64: string, mediaType: string, voiceNote: string | null) {
  ...
  const userContent: any[] = [
    ...
    },
    {
      type: 'text',
      text: voiceNote
        ? `Extract all data from this finish/center slip.\n\nUser voice note for context: "${voiceNote}"`
        : 'Extract all data from this finish/center slip.',
    },
  ]
...
// POST -- extract finish slip data from image
