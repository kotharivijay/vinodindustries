...
const QUERY_FUNCTIONS: Record<string, (args: any) => Promise<any>> = {
  ...
  missing_slips: async ({ type }: any) => {
    ...
    if (t === 'folding' || t === 'fr' || t === 'folding receipt') {
      ...
      const nums: number[] = entries.map((e: any) => parseInt(e.slipNo)).filter((n: number) => !isNaN(n)).sort((a: number, b: number) => a - b)
    ...
    if (t === 'fold' || t === 'fold program') {
      ...
      const nums: number[] = entries.map((e: any) => parseInt(e.foldNo)).filter((n: number) => !isNaN(n)).sort((a: number, b: number) => a - b)
      const unique: number[] = [...new Set(nums)]
