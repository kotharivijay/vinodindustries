...
export default function TokenClient({ captureSecret, appOrigin }: { captureSecret: string; appOrigin: string }) {
  ...
  // origin works at runtime too; SSR passed in captureSecret + fallback origin
  const origin = typeof window !== 'undefined' ? window.location.origin : appOrigin
  const snippet = `(() => {
  const au = sessionStorage.getItem('authUser');
  if (!au) { alert('Open payroll.petpooja.com and log in first.'); return }
  fetch('${origin}/api/attendance/save-token', {
    method: 'POST',
    headers: {
    ...
    console.log(d);
    alert(d.ok ? 'Saved — expires ' + new Date(d.expiresAt).toLocaleString() + ' (' + d.daysLeft + ' days)' : 'Error: ' + (d.error || JSON.stringify(d)))
    ...
    <div className="p-4 md:p-8 max-w-3xl">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 mb-4">
        ...
        <ol className="text-xs text-gray-600 dark:text-gray-300 space-y-1 mb-3 list-decimal list-inside">
          ...
          <li>Open DevTools (F12) → <b>Console</b> tab on the Petpooja tab.</li>
          <li>Click <b>Copy snippet</b> below → paste into the Console → Enter.</li>
          <li>An alert will confirm token saved with expiry date.</li>
        </ol>
        <pre className="bg-gray-900 text-green-300 text-[11px] p-3 rounded-lg overflow-auto max-h-60 whitespace-pre">{snippet}</pre>
        <button onClick={() => { navigator.clipboard.writeText(snippet); alert('Copied! Paste into DevTools Console on payroll.petpooja.com.') }}
          className="mt-2 text-xs bg-indigo-600 text-white px-3 py-1.5 rounded-lg font-semibold">Copy snippet</button>
        <p className="mt-2 text-[10px] text-gray-400">Snippet includes a one-time capture key; safe to share within your team since it only lets the bearer save tokens to THIS app.</p>
      </div>
