...
export default function DyeingForm() {
  ...
  const [lotWeights, setLotWeights] = useState<{ lotNo: string; weightPerThan: number; kgPerMtr: number; grayMtr: number }[]>([])
    ...
    <div className="p-4 md:p-8 max-w-3xl">
      ...
      {showResume && (
        <div className="bg-amber-50 border border-amber-300 rounded-xl p-4 mb-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700 mb-3">Your images were saved to cloud. You can continue right where you left off.</p>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={discardDraft}
                className="border border-amber-300 text-amber-700 px-3 py-2 rounded-lg text-sm hover:bg-amber-100">
                Discard
              </button>
            </div>
          </div>
      ...
      {queueTotal > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
          ...
          {uploadingDrafts && (
            <p className="text-xs text-purple-500 mb-2 animate-pulse">☁ Saving to cloud — you can resume later if you exit...</p>
          )}
          {!uploadingDrafts && cloudSaved && (
            <p className="text-xs text-green-600 mb-2">☁ Saved to cloud — safe to exit and resume later</p>
          )}
          ...
          <div className="flex flex-col justify-between gap-2 flex-1 min-w-[160px]">
            ...
            <div className="border border-gray-100 rounded-lg overflow-hidden">
              <button
                ...
                className="flex items-center gap-2 w-full px-3 py-2 text-xs bg-gray-50 hover:bg-gray-100 text-gray-600 transition"
              >
                <span className="font-medium">🏷 Chemical Tags</span>
                {tags.length > 0 && (
                  <span className="bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded text-[10px] font-semibold">{tags.length}</span>
                )}
                <span className="ml-auto text-gray-400">{showTagPanel ? '▲' : '▼'}</span>
              </button>

              {showTagPanel && (
                <div className="p-3 space-y-2">
                  <p className="text-[10px] text-gray-400">Short names the AI expands in voice notes. Auto-learned from your extractions.</p>

                  {tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {tags.map((t, i) => (
                        <div key={i} className="flex items-center gap-1 bg-indigo-50 border border-indigo-200 rounded-full px-2 py-0.5 text-xs">
                          <span className="font-semibold text-indigo-700">{t.tag}</span>
                          <span className="text-indigo-400">→</span>
                          <span className="text-gray-700">{t.chemical}</span>
                          <button
                            type="button"
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <input
                      ...
                      className="border border-gray-200 rounded-lg px-2 py-1 text-xs w-28 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                    />
                    ...
                    <input
                      ...
                      list="tag-chem-list"
                      className="border border-gray-200 rounded-lg px-2 py-1 text-xs flex-1 min-w-[120px] focus:outline-none focus:ring-1 focus:ring-indigo-400"
                    />
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-4">
          ...
          {processes.length > 0 && (
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xs font-medium text-gray-500">Process Presets:</span>
