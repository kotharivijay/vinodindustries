  ...
  return (
    <div className="p-4 md:p-8 max-w-xl">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-800">New Dyeing Slip Entry</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Lot No *" span={2}>
            ...
            />
            {stockStatus === 'loading' && (
              <p className="text-xs text-gray-400 mt-1">Checking grey stock...</p>
            )}
            {stockStatus === 'not_found' && (
              <p className="text-xs text-red-500 mt-1">⚠ Lot not found in Grey register</p>
            )}
            {stockStatus === 'no_stock' && stockInfo && (
              <p className="text-xs text-amber-600 mt-1">
                ⚠ No stock available — Grey: {stockInfo.greyThan}, Despatched: {stockInfo.despatchThan}, Balance: <strong>{stockInfo.stock}</strong>
              </p>
            )}
