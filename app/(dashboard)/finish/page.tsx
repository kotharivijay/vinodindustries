...
export default function FinishListPage() {
  ...
  const fi = 'w-full border border-gray-200 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300 mt-1'

  function SortTh({ field, label, right }: { field: SortField; label: string; right?: boolean }) {
    const active = sortField === field
    return (
      <th onClick={() => toggleSort(field)}
        className={`px-3 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap cursor-pointer select-none hover:text-teal-600 group ${right ? 'text-right' : 'text-left'}`}>
        <span className={`flex items-center gap-1 ${right ? 'justify-end' : ''}`}>
          {label}
          <span className={active ? 'text-teal-600' : 'text-gray-300 group-hover:text-gray-400'}>
            {active ? (sortDir === 'asc' ? '\u2191' : '\u2193') : '\u2195'}
          </span>
        </span>
      </th>
                      ...
                      return (
                        <div key={e.id} className="p-4">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mb-1">
                            {lotsArr.map((lot, li) => (
                              <Link key={li} href={`/lot/${encodeURIComponent(lot.lotNo)}`} className="inline-flex items-center gap-1 bg-teal-50 text-teal-700 text-xs font-semibold px-2.5 py-1 rounded-full hover:bg-teal-100">
                          ...
                          {chemCount > 0 && (
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {totalCost > 0 && (
                                <span className="text-[10px] text-teal-600 bg-teal-50 px-1.5 py-0.5 rounded font-medium">&#8377;{totalCost.toFixed(0)}</span>
                              )}
                            </div>
                          )}
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 border-b">
                        <tr>
                          ...
                          <th className="px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap cursor-pointer hover:text-teal-600"
                          ...
                          <tr key={e.id} className="hover:bg-gray-50 transition">
                            ...
                            <td className="px-3 py-2.5">
                              <div className="flex flex-wrap gap-1">
                                {dLots.map((lot, li) => (
                                  <Link key={li} href={`/lot/${encodeURIComponent(lot.lotNo)}`} className="inline-flex items-center gap-1 bg-teal-50 text-teal-700 text-xs font-semibold px-2 py-0.5 rounded-full hover:bg-teal-100">
                                    {lot.lotNo} <span className="text-teal-400 font-normal">({lot.than})</span>
                                  </Link>
                                ))}
                              </div>
                            ...
                            <td className="px-3 py-2.5 whitespace-nowrap">
                              <button onClick={() => router.push(`/finish/${e.id}/edit`)} className="text-teal-500 hover:text-teal-700 text-xs font-medium mr-3">Edit</button>
