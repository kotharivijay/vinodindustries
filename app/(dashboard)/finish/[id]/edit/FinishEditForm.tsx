...
export default function FinishEditForm({ id }: { id: string }) {
  ...
  // Load entry data
  useEffect(() => {
    fetch(`/api/finish/${id}`).then(r => r.json()).then(e => {
      setForm({
  ...
  function updateLot(i: number, field: 'lotNo' | 'than' | 'meter', value: string) {
    setLots(prev => {
      const updated = [...prev]
  ...
  function addLotRow() {
    setLots(prev => [...prev, { lotNo: '', than: '', meter: '' }])
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/finish/${id}`, {
      method: 'PUT',
  ...
  return (
    <div className="p-4 md:p-8 max-w-xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-xl font-bold text-gray-800">Edit Finish Slip</h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button type="button" onClick={addLotRow} className="text-xs text-teal-600 hover:text-teal-800 font-medium">
              + Add Lot
            </button>
          </div>
                ...
                <div className="pl-7 grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400 bg-white"
                      value={lot.meter}
                      onChange={e => updateLot(i, 'meter', e.target.value)}
                      placeholder="Meters"
                    />
                  </div>
                </div>
          ...
          {lots.length > 1 && (
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
              ...
              <span className="text-sm font-bold text-emerald-600">
                {lots.reduce((s, l) => s + (parseInt(l.than) || 0), 0)} than
                {' / '}
                {lots.reduce((s, l) => s + (parseFloat(l.meter) || 0), 0).toFixed(1)} m
              </span>
            </div>
          )}
        </div>
