...
export default function KSIOutstandingPage() {
  ...
  const partyGroups = useMemo(() => {
    ...
    for (const b of bills) {
      ...
      if (!g) { g = { partyName: b.partyName, totalReceivable: 0, totalPayable: 0, billCount: 0, maxOverdue: 0 }; map.set(b.partyName, g) }
                    ...
                    {partyBills.map(b => (
                      <div key={b.id} className={`rounded-lg p-3 border text-xs ${b.type === 'receivable' ? 'bg-green-900/20 border-green-800' : 'bg-rose-900/20 border-rose-800'}`}>
                        ...
                        <div className="flex flex-wrap gap-x-4 mt-1 text-gray-500">
                          ...
                          {b.overdueDays > 0 && <span className={`font-medium ${b.overdueDays > 90 ? 'text-red-400' : 'text-amber-400'}`}>{b.overdueDays}d overdue</span>}
